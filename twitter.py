...
class Tweet(base.Item):
    ...
    outlinks = base._DeprecatedProperty('outlinks',
                                                 lambda self: [x.url for x in self.links] if self.links else [],
                                                 'links (url attribute)')
    outlinksss = base._DeprecatedProperty('outlinksss', lambda self: ' '.join(
        x.url for x in self.links) if self.links else '', 'links (url attribute)')
    tcooutlinks = base._DeprecatedProperty('tcooutlinks',
                                                    lambda self: [x.tcourl for x in self.links] if self.links else [],
                                                    'links (tcourl attribute)')
    tcooutlinksss = base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(
...
class User(base.Entity):
    ...
    descriptionUrls = base._DeprecatedProperty('descriptionUrls', lambda self: self.descriptionLinks,
                                                        'descriptionLinks')
    linkUrl = base._DeprecatedProperty('linkUrl', lambda self: self.link.url if self.link else None,
                                                'link.url')
    linkTcourl = base._DeprecatedProperty('linkTcourl', lambda self: self.link.tcourl if self.link else None,
                                                   'link.tcourl')
    description = base._DeprecatedProperty('description', lambda self: self.renderedDescription,
                                                    'renderedDescription')
...
class _CLIGuestTokenManager(GuestTokenManager):
    def __init__(self):
        ...
        dir = os.path.join(cacheHome, 'ZinniaProject')
...
class TwitterHashtagScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('hashtag', type=base.nonempty_string('hashtag'),
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('list', type=base.nonempty_string('list'),
